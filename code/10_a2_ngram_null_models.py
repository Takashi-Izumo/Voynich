...
def main():
    ...
    rows=[]
    designs = (
        ('exact_length_conditioned_START_END', 'exact'),
        ('free_length_START_END', 'free'),
    )
    for design_label, design in designs:
        for mi,model in enumerate(('uni','bi','tri')):
            ...
            for r in range(args.runs):
                ...
                if design=='exact':
                    words=[exact(model,L,rng) for L in lens]; ll=list(lens)
                else:
                ...
                rows.append(dict(
                    ...
                    model=model,
                    run=r+1,
                    A2_token_rate=cc[0]/297,
                    HerbalA_outside_A2_token_rate=cc[1]/297,
                    VM_outside_HerbalA_token_rate=cc[2]/297,
                    Unattested_token_rate=cc[3]/297,
                    VM_total_token_rate=cc[:3].sum()/297,
                    Outside_A2_token_rate=(cc[1]+cc[2])/297,
                    generated_types=len(wc),
                    A2_type_rate=tc[0]/len(wc),
                    HerbalA_type_rate=(tc[0]+tc[1])/len(wc),
                    VM_type_rate=tc[:3].sum()/len(wc),
                    hapax=sum(v==1 for v in wc.values()),
                    mean_length=np.mean(ll),
                    sd_length=np.std(ll),
                    one_char_rate=(np.mean(np.asarray(ll)==1) if design=='free' else np.nan),
                    eight_plus_rate=(np.mean(np.asarray(ll)>=8) if design=='free' else np.nan),
                ))
    df=pd.DataFrame(rows)
    run_path=f'{args.out}_{args.runs}runs.csv'
    df.to_csv(run_path,index=False)
    metric_map = {
        'A2': 'A2_token_rate',
        'HerbalA_outside_A2': 'HerbalA_outside_A2_token_rate',
        'VM_outside_HerbalA': 'VM_outside_HerbalA_token_rate',
        'Unattested': 'Unattested_token_rate',
        'VM_total': 'VM_total_token_rate',
        'Outside_A2_total': 'Outside_A2_token_rate',
        'Generated_types': 'generated_types',
        'A2_type_rate': 'A2_type_rate',
        'HerbalA_type_rate': 'HerbalA_type_rate',
        'VM_type_rate': 'VM_type_rate',
        'Hapax': 'hapax',
        'Mean_length': 'mean_length',
        'SD_length': 'sd_length',
        'One_char_rate': 'one_char_rate',
        'Eight_plus_rate': 'eight_plus_rate',
    }
    sm=[]
    for (d,m),g in df.groupby(['design','model']):
        for metric,col in metric_map.items():
            ...
            if len(x)==0: continue
            sm.append(dict(design=d,model=m,metric=metric,mean=x.mean(),p2_5=np.quantile(x,.025),p97_5=np.quantile(x,.975)))
    pd.DataFrame(sm).to_csv(args.out+'_summary.csv',index=False)
    pd.DataFrame([
        {'corpus':'A2','tokens':len(a2),'types':len(set(a2))},
        {'corpus':'Herbal_A','tokens':len(ha),'types':len(set(ha))},
        {'corpus':'strict_whole_VM','tokens':len(vm),'types':len(set(vm))},
    ]).to_csv(args.out+'_corpus_audit.csv',index=False)
